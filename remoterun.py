...
# Settings
py_files = ['main.py','fileDownloader.py','FileScanner.py','message.py', 'SharedFile.py', 'tcpListener.py', 'tcpMessage.py', 'ticket.py']
#py_files = ['share/jiaoben.txt']
remote_python_interpreter = '/usr/local/bin/python3'
remote_current_working_directory = '/home/tc/workplace/cw2/'
remote_ip1 = '192.168.56.101'
remote_ip2 = '192.168.56.102'
remote_ip3 = '192.168.56.103'
remote_username = 'tc'
remote_password = '123'
...
def run1():
    ...
    try:
        ssh.connect(remote_ip1, username=remote_username, password=remote_password, port=22, timeout=5)
        ...
        for f in py_files:
            ...
            print(f'Send {components[-1]} to {remote_ip1}:{target_dir}')
...
def run2():
    ...
    ssh.set_missing_host_key_policy(AutoAddPolicy())
    try:
        ssh.connect(remote_ip2, username=remote_username, password=remote_password, port=22, timeout=5)
        # mount /mnt/sda1 to home folder as "workplace"
        ssh.exec_command(f'if [ ! -d "workplace" ]; then\nmkdir -p workplace\necho {remote_password} | sudo mount /mnt/sda1 ~/workplace\nfi')
        # change the ownership of workplace folder
        ssh.exec_command(f'echo {remote_password} | sudo chown tc /home/tc/workplace')
        ...
        for f in py_files:
            ...
            if len(components) > 1:  # Files in folders
                ...
                target_dir = remote_current_working_directory.replace('\\', '/')
            print(f'Send {components[-1]} to {remote_ip2}:{target_dir}')
            sftp.put(f, join(target_dir, components[-1]).replace('\\', '/'))
...
def run3():
    ...
    ssh.set_missing_host_key_policy(AutoAddPolicy())
    try:
        ssh.connect(remote_ip3, username=remote_username, password=remote_password, port=22, timeout=5)
        # mount /mnt/sda1 to home folder as "workplace"
        ssh.exec_command(f'if [ ! -d "workplace" ]; then\nmkdir -p workplace\necho {remote_password} | sudo mount /mnt/sda1 ~/workplace\nfi')
        # change the ownership of workplace folder
        ssh.exec_command(f'echo {remote_password} | sudo chown tc /home/tc/workplace')
        ...
        for f in py_files:
            ...
            if len(components) > 1:  # Files in folders
                ...
                target_dir = remote_current_working_directory.replace('\\', '/')
            print(f'Send {components[-1]} to {remote_ip3}:{target_dir}')
            sftp.put(f, join(target_dir, components[-1]).replace('\\', '/'))
